...
class LockerFragment : BaseFragment<FragmentLockerBinding>(FragmentLockerBinding::inflate) {
    ...
    override fun initAfterBinding() {
        // 더미데이터랑 Adapter 연결
        var dumdata = ArrayList<Cocktail_locker>()
        dumdata.add(Cocktail_locker(
            "스트로베리 다이키리",
            "Strawberry RR",
        ...
        dumdata.add(Cocktail_locker(
            ...
            "이사하고, 자취방때메, 너무바빠, 죽겠다어우"
        ))

        val cocktailRecyclerViewAdapter = LockerRVAdapter(dumdata)
        // 리사이클러뷰에 어댑터를 연결
        binding.lockerCocktailListRv.adapter = cocktailRecyclerViewAdapter
        selectCocktailByCocktail(dumdata[0])
